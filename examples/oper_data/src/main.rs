...
fn print_help(program: &str) {
    println!("Usage: {} <module-to-provide-data-from> <path-to-provide>", program);
}

/// Notification callback.
extern "C" fn dg_get_items_cb(session: *mut sr_session_ctx_t,
                              module_name: *const c_char, xpath: *const c_char,
                              _request_xpath: *const c_char, _request_id: u32,
                              parent: *mut *mut lyd_node, _private_data: *mut c_void) -> i32 {
    let xpath: &CStr = unsafe { CStr::from_ptr(xpath) };
    let module_name: &CStr = unsafe { CStr::from_ptr(module_name) };

    println!("");
    println!("");
    println!(r#" ========== DATA FOR "{}" "{}" REQUESED ======================="#,
             module_name.to_str().unwrap(), xpath.to_str().unwrap());
    println!("");
    ...
    if module_name.to_str().unwrap() == "examples" && xpath.to_str().unwrap() == "/examples:stats" {
        ...
        unsafe {
            ...
            *parent = lyd_new_path(null_ptr as *mut lyd_node, context,
                                   path1.as_ptr() as *const _ as *const i8,
                                   val1.as_ptr() as *const _ as *mut c_void,
                                   0, 0);
            lyd_new_path(*parent, null_ptr as *const ly_ctx,
                         path2.as_ptr() as *const _ as *const i8,
                         val2.as_ptr() as *const _ as *mut c_void, 0, 0);
        }
    }
...
fn main() {
    ...
    println!(r#"Application will provide data "{}" of "{}"."#, path, mod_name);
    ...
    loop {
        ...
        unsafe {
            ...
            rc = sr_oper_get_items_subscribe(session, mod_name, path, Some(dg_get_items_cb),
